...
const Detaileventngo = () => {
    ...
    const deleteEvent = async () => {
        try {
            const response = await axios.delete(`http://localhost:9002/deleteevent/${eventId}`);
        ...
        if (Event.edate > today.toISOString()) {
            return (
                <>
                    ...
                    <br />
                    <button className="btn btn-outline-warning" onClick={deleteEvent} >Delete Event</button>
                    </div>
                </>
            )
        }
